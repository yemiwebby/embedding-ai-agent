...
if [ ! -f "app/main.py" ]; then
    echo "❌ Please run this script from the circle_ci_agent_demo directory"
    exit 1
fi

# Check if OpenAI API key is set
if [ -z "$OPENAI_API_KEY" ]; then
    echo "⚠️  OPENAI_API_KEY not set. AI analysis will be skipped."
...
if [ "$SKIP_AI" = false ]; then
    ...
    echo ""
    echo "⏭️  Skipping AI analysis (OpenAI API key not configured)"
fi
...
if [ "$SKIP_AI" = false ]; then
    ...
    echo "  4. ⏭️  AI analysis skipped (configure OPENAI_API_KEY to enable)"
fi
echo ""
